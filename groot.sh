...
info_array[0]=""
info_array[1]="WELCOME GROOT"
info_array[2]="==============================================="
info_array[3]="${dcolor}Date        ${color}$(date +"%A %d %B %Y %H:%M:%S" | sed 's/.*/\u&/')"
info_array[4]="${dcolor}Uptime      ${color}$(uptime -p | sed s/'up '/''/)"
info_array[5]="${dcolor}Release     ${color}$(lsb_release -d -s)"
info_array[6]="${dcolor}Bash        ${color}$(echo $BASH_VERSION | cut -d '(' -f 1)"
info_array[7]="${dcolor}Python      ${color}$(python --version | cut -d " " -f 2)"
info_array[8]="${dcolor}CPU load    ${color}$(cpu_mem_usage $(top -bn 1 |grep "Cpu(s)" | awk '{print int($2+$6)}'))"
info_array[9]="${dcolor}MEM load    ${color}$(cpu_mem_usage $(free -t | awk 'NR == 2 {print int($3/$2*100)}'))"
# info_array[10]="${dcolor}Gemini      ${color}$(bash geminiflake.sh summary)"
info_array[11]=""
info_array[12]=""
...
echo -e "${dcolor}\nDISK USAGE${NC}"
# disk usage function
array_disk=("/" "/mnt/c" "/mnt/g")
for disk in "${array_disk[@]}"
do
  disk_usage ${disk}
done
